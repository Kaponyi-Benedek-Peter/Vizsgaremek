    ...
    internal class upload_post_image_admin
    {
        public static void main(HttpListenerContext data, string lenyeg)
        {
            ...
            try
            {
                ...
                service.shared.log($"Image upload request: {admin_id} : post {post_id} ({data.Request.RemoteEndPoint.Address})");

                string saved_filename = "";
                int resp = service.upload_post_image_admin.process_upload_post_image(admin_id, sesstoken, post_id, image_b64, out saved_filename, transparency);
            ...
            {
                service.shared.log($"Error 1: {ex.Message} --controller.upload_post_image_admin.main");
                var respon = new { status = "malformed_request", statuscode = "400" };
                data.Response.StatusCode = 400;
                byte[] buffer = Encoding.UTF8.GetBytes(JsonSerializer.Serialize(respon));
                data.Response.OutputStream.Write(buffer, 0, buffer.Length);
