...
namespace Servo.service
{
    internal class unban_user
    {
        ...
        public static int process_unban_user(string controller_admin_id, string controller_sesstoken, string controller_target_user_id)
        {
            ...
            if (model_sesstoken == controller_sesstoken && (admin_accstate == "admin" || admin_accstate == "superadmin"))
            {
                ...
                {
                    // nem lehet magát unbannolni
                    return 422;
                }
                else if (target_accstate == "verified" && (admin_accstate== "admin" && admin_accstate == "superadmin"))
                {

                    //do
                    int result = model.shared.update_account_state_by_id(controller_target_user_id, "banned");
