...
namespace Servo.service
{
    internal class chpass_request
    {
        ...
        public static int sendchpass(string email, string id, string newpasshash)
        {
        ...
        public static int process_chpas_request(string controller_email,string controller_jelszo_uj,string ip)
        {
            try
            {
                ...
                if (accstate == "verified")
                {
                    ...
                    if (result1 == 200)
                    {
                        if(sendchpass(controller_email, controller_id, service.shared.hashpass(controller_jelszo_uj)) == 200)
                        {
                            return 200;
                        }
                        else
