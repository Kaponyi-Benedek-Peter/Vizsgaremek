...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_product_status_by_id` (IN `p_id` INT)   BEGIN
SELECT status from roy.products
where id = p_id;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_product_status_by_id` (IN `p_id` INT, IN `p_new_status` TINYINT)   BEGIN
	UPDATE roy.products
